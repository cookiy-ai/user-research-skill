...
```
node scripts/cookiy.js study guide get --study-id <uuid>
```
...
```
node scripts/cookiy.js study guide wait --study-id <uuid>
```
...
```
node scripts/cookiy.js study guide update --study-id <uuid> --base-revision <s> --idempotency-key <s> [--change-message <s>] --json '<patch>'
```
