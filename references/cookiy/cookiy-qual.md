...
```
node scripts/cookiy.js study list [--limit <n>] [--cursor <s>]
```
...
```
node scripts/cookiy.js study create --query <s> [--thinking <s>] [--attachments <s>]
```
...
```
node scripts/cookiy.js study upload --content-type <s> (--image-data <s> | --image-url <s>)
```
...
```
node scripts/cookiy.js study status --study-id <uuid>
```
